...
type Skill struct {
	ID      uint64    `json:"skillID" xorm:"not null default nextval('skill_id_seq'::regclass) pk BIGINT autoincr 'id'"`
	...
	Owner   string   `json:"owner" xorm:"not null index index(skill_owner_is_open_idx) VARCHAR(20)"` //鸟币号ID，必填
	...
	Desc    string   `json:"desc,omitempty" xorm:"index VARCHAR(5000)"`                              //技能描述，少于5000个字符
	...
	IsOpen bool `json:"isOpen" xorm:"not null default true index(skill_owner_is_open_idx) BOOL"` //上架或下架
}
